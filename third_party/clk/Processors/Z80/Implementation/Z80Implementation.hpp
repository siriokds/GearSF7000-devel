...
template <	class T,
	...
	const auto advance_operation = [&] {
		pc_increment_ = 1;
		bool preserve_ld_a_ir_interrupt_pv_quirk = false;
		if(last_request_status_) {
			...
			if(last_request_status_ & (Interrupt::PowerOn | Interrupt::Reset)) {
				...
				preserve_ld_a_ir_interrupt_pv_quirk = true;
			}
		} else {
			current_instruction_page_ = &base_page_;
			scheduled_program_counter_ = base_page_.fetch_decode_execute_data;
		}
		if(!preserve_ld_a_ir_interrupt_pv_quirk) {
			ld_a_ir_interrupt_pv_quirk_ = false;
		}
	};
	...
	while(1) {
		...
		while(true) {
			// Do not execute zero-time state changes from the next opcode,
			// reset or interrupt sequence after the caller's time budget has
			// ended exactly at an externally visible execution boundary.
			if(!number_of_cycles_ && is_at_execution_boundary()) return;
			...
			switch(operation->type) {
				...
				case MicroOp::DecodeOperation:
					...
					// IM 0 supplies the opcode during interrupt acknowledge, so
					// BeginIRQMode0 suppresses the normal M1 PC increment. That
					// suppression applies only to the supplied opcode byte: any
					// following immediate/displacement bytes are ordinary memory
					// fetches and must advance PC normally.
					pc_increment_ = 1;
...
// MARK: - Interrupt state
				...
				case MicroOp::EI:
					...
					// A real Z80 suppresses maskable-interrupt acceptance during
					// the latter half of every EI opcode fetch. Consequently an
					// uninterrupted run of EI opcodes remains atomic and the IRQ
					// may be accepted only after the instruction following the
					// final EI. request_status_ is the live level; keep it pending.
					// last_request_status_ is CLK's already-sampled boundary; clear
					// only that stale sample to model EI's suppression at this
					// boundary. RESET and NMI samples are intentionally untouched.
					last_request_status_ &= ~Interrupt::IRQ;
				break;

				case MicroOp::DI:
					iff1_ = iff2_ = false;
					request_status_ &= ~Interrupt::IRQ;
					// DI takes effect before interrupt acceptance at this
					// instruction boundary. Discard an IRQ level sampled during
					// the opcode fetch as well as the live pending request.
					last_request_status_ &= ~Interrupt::IRQ;
				break;
...
// MARK: - Input and Output
				...
				case MicroOp::SetAFlags:
					...
					sign_result_ = zero_result_ = bit53_result_ = a_;
					set_did_compute_flags();
					ld_a_ir_interrupt_pv_quirk_ = true;
				break;
...
// MARK: - Special-case Flow
				...
				case MicroOp::BeginIRQ:
					// Zilog documents this NMOS behaviour explicitly: if a
					// maskable interrupt is accepted immediately after LD A,I or
					// LD A,R, P/V is cleared even though IFF2 was set when the
					// instruction began. Floooh's trace explains why: the flags
					// settle in the following fetch, alongside interrupt entry.
					if(ld_a_ir_interrupt_pv_quirk_) {
						parity_overflow_result_ = 0;
						ld_a_ir_interrupt_pv_quirk_ = false;
					}
					iff2_ = iff1_ = false;
					request_status_ &= ~Interrupt::IRQ;
