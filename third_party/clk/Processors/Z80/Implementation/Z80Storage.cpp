...
void ProcessorStorage::install_default_instruction_set() {
	...
	MicroOp nmi_program[] = {
		...
		BusOp(ReadOpcodeEnd()),
		// NMI performs an M1 memory-read/refresh cycle (the data bus is
		// ignored), so its refresh counter side effect is the same as any
		// other M1 fetch.
		{ MicroOp::IncrementR },
		BusOp(Refresh()),
		InternalOperation(2),
	...
	MicroOp irq_mode0_program[] = {
		...
		{ MicroOp::IncrementR },
		// IM 0 has the same 2T refresh tail as the maskable-interrupt
		// acknowledge used by IM 1/2. The acknowledged byte replaces the
		// opcode fetch, but the complete acknowledge cycle is still 6T.
		BusOp(Refresh()),
...
#define NOP_ROW()	NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP, NOP
	std::unique_ptr<InstructionTableStorage> ed_program_table(new InstructionTableStorage {{
		NOP_ROW(),	/* 0x00 */
		NOP_ROW(),	/* 0x10 */
		NOP_ROW(),	/* 0x20 */
		NOP_ROW(),	/* 0x30 */
	...
	assemble_page(target, ed_program_table->table, false);
#undef NOP_ROW
}
...
#define CB_PAGE(m, p)	m(RLC), m(RRC),	m(RL),	m(RR),	m(SLA),	m(SRA),	m(SLL),	m(SRL),	OCTO_OP_GROUP(p, BIT),	OCTO_OP_GROUP(m, RES),	OCTO_OP_GROUP(m, SET)
	...
	std::unique_ptr<InstructionTableStorage> cb_program_table(new InstructionTableStorage {{
	...
	std::unique_ptr<InstructionTableStorage> offsets_cb_program_table(new InstructionTableStorage {{
		CB_PAGE(IX_MODIFY_OP_GROUP, IX_READ_OP_GROUP)
	}});
	assemble_page(target,
		add_offsets ? offsets_cb_program_table->table : cb_program_table->table,
		add_offsets);
...
#define DEC_INC_DEC_LD(rf, r)	\
	...
	std::unique_ptr<InstructionTableStorage> base_program_table(new InstructionTableStorage {{
	...
	if(add_offsets) {
		...
		MicroOp copy_program[] =
			Sequence(FINDEX(), ReadInc(pc_, temp8_), InternalOperation(4), Write(INDEX_ADDR(), temp8_))
		;
		std::copy(std::begin(copy_program), std::end(copy_program),
			std::begin(base_program_table->table[0x36]));
	}

	assemble_cb_page(cb_page, index, add_offsets);
	assemble_page(target, base_program_table->table, add_offsets);
}
...
bool ProcessorBase::is_at_execution_boundary() const {
	if(!scheduled_program_counter_) return true;
	if(is_starting_new_instruction()) return true;
	if(scheduled_program_counter_ == reset_program_.data()) return true;
	if(scheduled_program_counter_ == nmi_program_.data()) return true;
	for(const auto &program : irq_program_) {
		if(scheduled_program_counter_ == program.data()) return true;
	}
	return false;
}
