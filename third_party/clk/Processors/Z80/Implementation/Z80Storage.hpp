...
protected:
	...
	// The NMOS Z80 updates LD A,I/R flags late enough that an immediately
	// accepted maskable interrupt clears IFF2 before P/V settles. Keep this
	// one-boundary latch so the interrupt response can reproduce that
	// documented silicon behaviour without delaying the other visible flags.
	bool ld_a_ir_interrupt_pv_quirk_ = false;
	...
	*/
	...
	struct InstructionTableStorage {
	...
	// GearSF7000 addition: grants the save-state serializer access to the
	// micro-op scheduler, so a snapshot can be taken part-way through an
	// instruction rather than only at instruction boundaries.
	friend struct GearSF7000StateAccess;
};
