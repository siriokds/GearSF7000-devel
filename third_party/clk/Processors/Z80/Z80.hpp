...
public:
	...
	*/
	...
	/*!
		@returns @c true if execution is at a debugger-visible boundary: before
		an opcode fetch or before a RESET, NMI or IRQ acknowledge sequence.

		Unlike is_starting_new_instruction(), this keeps an interrupt acknowledge
		separate from the instruction that precedes it when single stepping.
	*/
	bool is_at_execution_boundary() const;
};
