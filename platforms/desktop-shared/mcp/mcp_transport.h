...
#define MCP_HTTP_AUTH_ENV "GEARSF7000_MCP_HTTP_TOKEN"
...
// The HTTP transport serves one client at a time and only accepts a new
// connection once the current one is closed.  If a request never produces a
// response the accept loop would wait forever, the listen backlog would fill,
// and macOS answers a full backlog with a refusal: the port looks dead while
// the emulator is still running.  Nothing may hold a client longer than this.
#ifndef MCP_HTTP_CLIENT_STALL_TIMEOUT_MS
#define MCP_HTTP_CLIENT_STALL_TIMEOUT_MS 30000
#endif
#ifndef MCP_HTTP_CLIENT_WAIT_SLICE_MS
#define MCP_HTTP_CLIENT_WAIT_SLICE_MS 500
#endif
...
// STDIO is a JSON-RPC stream: even debug logging must never share stdout.
// HTTP builds can keep using the emulator's normal diagnostic logger.
extern bool g_mcp_stdio_mode;
#define MCP_TRANSPORT_LOG(...) do { if (!g_mcp_stdio_mode) Log(__VA_ARGS__); } while (0)
#define Error(...) MCP_TRANSPORT_LOG(__VA_ARGS__)
#undef Debug
#define Debug(...) MCP_TRANSPORT_LOG(__VA_ARGS__)
...
typedef u64 McpRequestToken;
#define MCP_REQUEST_TOKEN_NONE ((McpRequestToken)0)

struct McpTransportStatus
{
    std::string mode;
    ...
    u64 activeRequestAgeMs = 0;
    u64 acceptedRequests = 0;
    u64 stallTimeouts = 0;
    u64 receiveTimeouts = 0;
    u64 clientDisconnects = 0;
    u64 invalidFraming = 0;
    u64 oversizedHeaders = 0;
    u64 oversizedBodies = 0;
    u64 authenticationRejects = 0;
    u64 discardedResponses = 0;
    u64 sendFailures = 0;
};
...
public:
    ...
    virtual bool send(const std::string& jsonLine, McpRequestToken request_token) = 0;
    virtual bool acknowledge_notification(McpRequestToken request_token) = 0;
    virtual bool reject_notification(McpRequestToken request_token) = 0;
    virtual bool validate_protocol_version(const std::string& method, McpRequestToken request_token) = 0;
    virtual void set_protocol_version(const std::string& version) = 0;
    virtual bool recv(std::string& jsonLine, McpRequestToken& request_token) = 0;
    virtual void close() = 0;
    virtual McpTransportStatus get_status() const = 0;

    // Releases the connection that owns request_token without answering it.
    // A transport-internal failure may pass NONE before recv() has returned a
    // token. Called when a response can no longer be produced, so the transport
    // does not stay blocked on a request nobody will ever finish.
    virtual void abort_request(McpRequestToken request_token) { (void)request_token; }
};
...
public:
    ...
    bool send(const std::string& jsonLine, McpRequestToken request_token)
    {
        (void)request_token;
    ...
    bool acknowledge_notification(McpRequestToken request_token)
    {
        (void)request_token;
        std::lock_guard<std::mutex> lock(m_mutex);
    ...
    bool reject_notification(McpRequestToken request_token)
    {
        return acknowledge_notification(request_token);
    }

    bool validate_protocol_version(const std::string& method, McpRequestToken request_token)
    {
        (void)method;
        (void)request_token;
        return true;
    ...
    bool recv(std::string& jsonLine, McpRequestToken& request_token)
    {
        while (!m_closed.load())
        {
            ...
            if (line_end != std::string::npos)
            {
                ...
                request_token = ++m_next_request_token;
    ...
    McpTransportStatus get_status() const
    {
        McpTransportStatus status;
        status.mode = "stdio";
        status.listening = !m_closed.load();
        return status;
    }
...
private:
    ...
    bool m_input_eof;
    McpRequestToken m_next_request_token = MCP_REQUEST_TOKEN_NONE;
};
...
public:
    HttpTransport(const std::string& address, int port)
    {
        ...
        m_active_request.store(false);
        m_active_request_started_ms.store(0);
        m_port = port;
        ...
        m_current_request_token = MCP_REQUEST_TOKEN_NONE;
        m_next_request_token = MCP_REQUEST_TOKEN_NONE;
        ...
        // A full backlog is refused outright on Darwin, so a brief stall in the
        // accept loop must not be enough to make the port look dead.
        ...
        MCP_TRANSPORT_LOG("[MCP] HTTP server listening on http://%s:%d%s", m_bind_address.c_str(), port, MCP_HTTP_ENDPOINT_PATH);
        if (!m_auth_token.empty())
            MCP_TRANSPORT_LOG("[MCP] HTTP bearer token authentication enabled from %s", MCP_HTTP_AUTH_ENV);
    }
    ...
    bool send(const std::string& jsonLine, McpRequestToken request_token)
    {
        return send_http_response(200, "OK", "application/json", jsonLine, true, NULL, request_token);
    }

    bool acknowledge_notification(McpRequestToken request_token)
    {
        return send_http_response(202, "Accepted", NULL, "", true, NULL, request_token);
    }

    bool reject_notification(McpRequestToken request_token)
    {
        return send_http_response(400, "Bad Request", NULL, "", true, NULL, request_token);
    }

    void abort_request(McpRequestToken request_token)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        // NONE is reserved for a transport failure thrown before recv() could
        // return an identity to ReaderLoop. Once recv() returns successfully,
        // every caller must carry the exact nonzero token it received.
        if (request_token == MCP_REQUEST_TOKEN_NONE)
            close_current_client_locked();
        else if (request_token == m_current_request_token)
            close_current_client_locked();
    }
    ...
    McpTransportStatus get_status() const
    {
        McpTransportStatus status;
        status.mode = "http";
        status.bindAddress = m_bind_address;
        status.port = m_port;
        status.listening = m_listening.load() && !m_closed.load();
        status.authenticationEnabled = !m_auth_token.empty();
        status.activeRequest = m_active_request.load();
        const u64 started = m_active_request_started_ms.load();
        const u64 now = monotonic_milliseconds();
        status.activeRequestAgeMs = status.activeRequest && started <= now ? now - started : 0;
        status.acceptedRequests = m_accepted_requests.load();
        status.stallTimeouts = m_stall_timeouts.load();
        status.receiveTimeouts = m_receive_timeouts.load();
        status.clientDisconnects = m_client_disconnects.load();
        status.invalidFraming = m_invalid_framing.load();
        status.oversizedHeaders = m_oversized_headers.load();
        status.oversizedBodies = m_oversized_bodies.load();
        status.authenticationRejects = m_authentication_rejects.load();
        status.discardedResponses = m_discarded_responses.load();
        status.sendFailures = m_send_failures.load();
        return status;
    }
    ...
    bool validate_protocol_version(const std::string& method, McpRequestToken request_token)
    {
        ...
        send_http_response(400, "Bad Request", NULL, "", true, NULL, request_token);
        return false;
    }
    ...
    bool configure_client_socket_timeouts(socket_t client)
    {
        ...
        // Darwin reports a closed peer as SIGPIPE by default.  A malformed or
        // impatient HTTP client must only lose its request, never the emulator.
#ifdef SO_NOSIGPIPE
        int no_sigpipe = 1;
        if (setsockopt(client, SOL_SOCKET, SO_NOSIGPIPE, &no_sigpipe, sizeof(no_sigpipe)) != 0)
            return false;
#endif
        return true;
#endif
    ...
    bool send_http_response(int code, const char* status, const char* content_type, const std::string& body, bool include_cors,
                            const char* extra_headers = NULL, McpRequestToken request_token = MCP_REQUEST_TOKEN_NONE)
    {
        std::lock_guard<std::mutex> lock(m_mutex);
        if (request_token != MCP_REQUEST_TOKEN_NONE && request_token != m_current_request_token)
        {
            m_discarded_responses.fetch_add(1);
            Debug("[MCP] Discarding response for expired HTTP request %llu (current=%llu)",
                  (unsigned long long)request_token,
                  (unsigned long long)m_current_request_token);
            return false;
        }
        ...
        while (total_sent < response.length())
        {
            int sent = ::send(m_current_client, response.c_str() + total_sent,
                              (int)(response.length() - total_sent), get_send_flags());
            if (sent <= 0)
            {
                m_send_failures.fetch_add(1);
    ...
    bool validate_and_reject_bearer_auth(const std::string& request)
    {
        ...
        {
            m_authentication_rejects.fetch_add(1);
    ...
    bool recv(std::string& jsonLine, McpRequestToken& request_token)
    {
        request_token = MCP_REQUEST_TOKEN_NONE;
        while (!m_closed.load())
        {
            socket_t server_socket = INVALID_SOCKET_VALUE;
            {
                std::unique_lock<std::mutex> lock(m_mutex);
                while (!m_closed.load() && m_current_client != INVALID_SOCKET_VALUE)
                {
                    m_client_cv.wait_for(lock, std::chrono::milliseconds(MCP_HTTP_CLIENT_WAIT_SLICE_MS), [this]
                    {
                    ...
                    long long held_ms = (long long)std::chrono::duration_cast<std::chrono::milliseconds>(
                        std::chrono::steady_clock::now() - m_client_started).count();

                    if (held_ms >= MCP_HTTP_CLIENT_STALL_TIMEOUT_MS)
                    {
                        m_stall_timeouts.fetch_add(1);
                        Error("[MCP] HTTP client held for %lldms without a response, dropping it to keep the port alive", held_ms);
                        close_current_client_locked();
                    }
                }
            ...
            McpRequestToken accepted_request_token = MCP_REQUEST_TOKEN_NONE;
            {
                std::lock_guard<std::mutex> lock(m_mutex);
                if (m_closed.load())
                {
                ...
                ++m_next_request_token;
                if (m_next_request_token == MCP_REQUEST_TOKEN_NONE)
                    ++m_next_request_token;
                m_current_request_token = m_next_request_token;
                accepted_request_token = m_current_request_token;
                m_accepted_requests.fetch_add(1);
                m_active_request.store(true);
                m_active_request_started_ms.store(monotonic_milliseconds());
                m_current_origin.clear();
                m_client_started = std::chrono::steady_clock::now();
            }
            ...
            {
                m_client_disconnects.fetch_add(1);
            ...
            if (request_timed_out || (read_error && !connection_closed))
            {
                if (request_timed_out)
                    m_receive_timeouts.fetch_add(1);
                close_current_client();
                continue;
            }
            ...
            if (header_too_large)
            {
                m_oversized_headers.fetch_add(1);
            ...
            if (body_too_large)
            {
                m_oversized_bodies.fetch_add(1);
            ...
            if (http_method == "POST")
            {
                ...
                {
                    std::lock_guard<std::mutex> lock(m_mutex);
                    // Preserve the identity assigned at accept. Do not turn a
                    // client that was closed concurrently during shutdown into
                    // a wildcard/NONE request, and do not rely on the current
                    // socket descriptor (which the OS is free to reuse).
                    if (accepted_request_token == MCP_REQUEST_TOKEN_NONE ||
                        accepted_request_token != m_current_request_token ||
                        m_current_client != client)
                    {
                        jsonLine.clear();
                    ...
                    request_token = accepted_request_token;
                }
...
private:
    static u64 monotonic_milliseconds()
    {
    ...
    std::atomic<bool> m_active_request;
    std::atomic<u64> m_active_request_started_ms;
    std::atomic<u64> m_accepted_requests{0};
    std::atomic<u64> m_stall_timeouts{0};
    std::atomic<u64> m_receive_timeouts{0};
    std::atomic<u64> m_client_disconnects{0};
    std::atomic<u64> m_invalid_framing{0};
    std::atomic<u64> m_oversized_headers{0};
    std::atomic<u64> m_oversized_bodies{0};
    std::atomic<u64> m_authentication_rejects{0};
    std::atomic<u64> m_discarded_responses{0};
    std::atomic<u64> m_send_failures{0};
    int m_port;
    ...
    McpRequestToken m_current_request_token;
    McpRequestToken m_next_request_token;
    std::chrono::steady_clock::time_point m_client_started;
};
