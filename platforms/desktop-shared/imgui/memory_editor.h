...
public:
    ...
    // A parsed element of an advanced find-bytes pattern: a fixed byte, a
    // single-byte wildcard ("??"), or a variable-length wildcard ("*")
    // bounded by m_find_bytes_wildcard_limit - built to find a routine that
    // has been relocated, where the opcodes match but an embedded address
    // operand does not.
    enum FindByteTokenType
    {
        FIND_TOKEN_BYTE,
        FIND_TOKEN_WILD_ONE,
        FIND_TOKEN_WILD_SPAN
    };

    struct FindByteToken
    {
        FindByteTokenType type;
        uint8_t value;
    };
    ...
    struct Options
    {
        ...
        // 0 = debugger fixed, 1 = Iosevka Condensed Regular,
        // 2 = Iosevka Condensed Light.
        int data_font = 0;
...
public:
    ...
    void SearchCapture();
    // narrow_previous=true re-checks only the addresses already in
    // m_search_results (comparing each against its own last recorded
    // value, not the original capture) instead of rescanning the whole
    // buffer - the progressive-narrowing "Next Scan" Cheat Engine-style
    // workflow. false (the default) is the original single-shot behavior:
    // a full scan against the SearchCapture() snapshot.
    int PerformSearch(int op, int compare_type, int compare_value, int data_type, bool narrow_previous = false);
    std::vector<Search>* GetSearchResults();
    int FindBytesSequence(const char* hex_str, int* out_addresses, int max_results);
    // Wildcard variant for the same use case as the panel's "Advanced"
    // checkbox: locating a routine that was relocated, where the opcodes
    // match but an embedded address operand does not. Pattern syntax is
    // "??" for one wildcard byte and "*" for a run of 0..wildcard_limit
    // wildcard bytes. out_lengths[i] is the matched byte length at
    // out_addresses[i] (varies per match when the pattern contains "*").
    int FindBytesSequenceAdvanced(const char* pattern, int wildcard_limit, int* out_addresses, int* out_lengths, int max_results);
...
private:
    ...
    void CalculateNarrowedSearchResults();
    ...
    bool ParseAdvancedPattern(const char* str, FindByteToken* tokens, int* out_count, int max_tokens);
    bool MatchAdvancedPattern(int data_pos, int total_bytes, const FindByteToken* tokens, int token_idx, int token_count, int wildcard_limit, int* out_end);
...
private:
    ...
    // Per-result match length in bytes, parallel to m_find_bytes_results -
    // needed in advanced mode because "*" makes matches variable-length, so
    // a single shared pattern length can no longer size the selection.
    std::vector<int> m_find_bytes_match_lengths;
    bool m_find_bytes_advanced;
    // Cap on how many bytes a single "*" may consume. Unbounded wildcards
    // over a whole memory buffer are what turn a two-star pattern into a
    // combinatorial search; this keeps it to what a relocated operand
    // (typically a 16-bit address) actually spans.
    int m_find_bytes_wildcard_limit;
};
